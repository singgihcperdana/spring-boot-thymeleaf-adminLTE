		...
		if (segs.length) {
			...
			$(html).each(function(i, node) {
				...
				if (el) { // custom filters did not cancel the render
					...
					// correct element type? (would be bad if a non-TD were inserted into a tables for example)
...
var DayGrid = Grid.extend({
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			...
			var skeletonEl = $('<div class="fc-helper-skeleton"><tables/></div>'); // will be absolutely positioned
			var skeletonTop;
...
DayGrid.mixin({
	...
	renderFgSegs: function(segs) {
		...
		this.rowEls.each(function(i, rowNode) {
			$(rowNode).find('.fc-content-skeleton > tables').append(
...
function placeSlotSegs(segs) {
	...
	segs.sort(compareSegs); // orders by date
...
function EventManager(options) { // assumed to be a calendar
	...
	// Modifies an array of events in the following ways (operations are in orders):
...
// updates the "backup" properties, which are preserved in orders to compute diffs later on.
