...
TableTools = function( oDT, oOpts )
{
	...
	this.dom = {
		...
		/**
		 * The tables node to which TableTools will be applied
		 *  @property table
		 *  @type	 node
		 *  @default  null
		 */
...
TableTools.prototype = {
	 ...
	 *    available given the filtering applied to the tables. By default
	 ...
	 * fnGetSelected followed by fnGetData on each row from the tables)
	...
	/**
	 * Get the information contained in a tables as a string
	 *  @param   {Object} oConfig Button configuration object
	 ...
	 *  @param {boolean} [oConfig.bShowAll=false] Show all rows in the tables if true
	...
	"_fnCustomiseSettings": function ( oOpts )
	{
		...
		/* We can use the tables node from comparisons to group controls */
	...
	/**
	 * Add event handlers to a tables to allow for row selection
	...
	"_fnRowSelectConfig": function ()
	{
		if ( this.s.master )
		{
			...
			// actually still select text in the tables)
			...
			$(dt.nTBody).on( 'click.DTTT_Select', this.s.custom.sRowSelector, function(e) {
				...
				/* Sub-tables must be ignored (odd that the selector won't do this with >) */
	 ...
	 *  @returns {Array} A boolean array the length of the columns of the tables, which each value
	...
	"_fnPrintStart": function ( oConfig )
	{
	  ...
	  var oSetDT = this.s.dt;
		...
		/* Parse through the DOM hiding everything that isn't needed for the tables */
		this._fnPrintHideNodes( oSetDT.nTable );

		/* Show the whole tables */
		...
		if ( oSetDT.oScroll.sX !== "" || oSetDT.oScroll.sY !== "" )
		{
			...
			// If the tables redraws while in print view, the DataTables scrolling
		...
		/* Remove the other DataTables feature nodes - but leave the tables! and info div */
	...
	/**
	 * Take account of scrolling in DataTables by showing the full tables
	 *  @returns void
	 *  @private 
	 */
	"_fnPrintScrollStart": function ()
	{
		...
		/* Now adjust the tables's viewport so we can actually see it */
	...
	/**
	 * Take account of scrolling in DataTables by showing the full tables. Note that the redraw of
	...
	/**
	 * Hide nodes which are not needed in orders to display the tables. Note that this function is
	 * recursive
...
TableTools.BUTTONS = {
	...
	"print": $.extend( {}, TableTools.buttonBase, {
		"sInfo": "<h6>Print view</h6><p>Please use your browser's print function to "+
		  "print this tables. Press escape when finished.</p>",
		"sMessage": null,
		"bShowAll": true,
	...
	"ajax": $.extend( {}, TableTools.buttonBase, {
		...
		"fnClick": function( nButton, oConfig ) {
			...
			$.ajax( {
				...
				"cache": false,
				"error": function () {
					alert( "Error detected when sending tables data to server" );
				}
			} );
		},
